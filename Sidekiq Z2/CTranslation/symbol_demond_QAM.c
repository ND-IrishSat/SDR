...
int* symbol_demod(double baseband_symbols[][], char *scheme[], double channel_gain, double preamble_len){

  float baseBandLen = sizeof(baseband_symbols) / sizeof(baseband_symbols[0][0]);
  
  float a_demodulated[baseBandLen+1000]; // fix this later
  float complex baseband_symbols_complex[baseBandLen + 1000];
  // i think these are vectors --> check with isaac
  float baseband_symbols_I = baseband_symbols[0];
  float baseband_symbols_Q = baseband_symbols[1];
  // check this 
  if(strcmp(scheme, "OOK") == 0){
    float s_on = 1.0 * channel_gain;
    float s_off = 0 * channel_gain;
    ...
    for (int i = 0; i < sizeof(baseband_symbols_complex)/sizeof(baseband_symbols_complex[0][0]); i++){
        if(cabs(baseband_symbols_complex[i]) - s_on) < cabs(baseband_symbols_complex[i] - s_off){
            a_demodulated[i] = 1;
        }
        else {
            a_demodulated[i] = 0;
  ...
  if(strcmp(scheme, "BPSK") == 0){
    a_demodulated[baseBandLen] = {0}; 
    float reference_plus = -1.0 * channel_gain;
    float reference_minus = -1.0 * reference_plus;
    for (int i= 0; i < strlen(baseband_symbols_complex); i++){
       baseband_symbols_complex[i] = baseband_symbols[i];
    }
    for (int i = 0; i < sizeof(baseband_symbols_complex)/sizeof(baseband_symbols_complex[0][0]); i++){
      if(cabs(baseband_symbols_complex[i] - reference_plus) < cabs(baseband_symbols_complex[i]) - reference_minus)) //not sure what goes here python code is kinda funky 
        {
          a_demodulated[sizeof(a_demodulated)/sizeof(a_demodulated[0]) + 1] = 0;
        }
        else{
          a_demodulated[sizeof(a_demodulated)/sizeof(a_demodulated[0]) + 1] = 1;
        }
      }
    }
   if (strcmp(scheme, "QPSK") == 0){
     // baseband symbols_I  
      for (int i = 0; i < sizeof(baseband_symbols)/sizeof(baseband_symbols[0][0]); i++){
        baseband_symbols_I[i] = baseband_symbols[0][i];
      }
      //baseband_symbols_Q 
     for (int i = 0; i < sizeof(baseband_symbols)/sizeof(baseband_symbols[0][0]); i++){
       baseband_symbols_Q[i] = baseband_symbols[1][i];
     }
     ...
     for (i = 0; i < sizeof(baseband_symbols_complex)/sizeof(baseband_symbols_complex[0][0]); i++){
  ...
  // change baseband_symbols_I[] in the following for loop to new_baseband_symbols_I[]
   ...
   // initialized I_demodulated and Q_demodulated again here but not sure way
    ...
    float reference_0000 = -3.0 * channel_gain - 3*j * channel_gain;
    float reference_0001 = -1.0 * channel_gain - 3*j * channel_gain;
    float reference_0010 = -3.0 * channel_gain - 3*j * channel_gain;
    float reference_0011 = 1.0 * channel_gain - 3*j * channel_gain;
    float reference_0100 = -3.0 * channel_gain - 1*j * channel_gain;
    float reference_0101 = -1.0 * channel_gain - 1*j * channel_gain;
    float reference_0110 = 3.0 * channel_gain - 1*j * channel_gain;
    float reference_0111 = 1.0 * channel_gain - 1*j * channel_gain;
    float reference_1000 = -3.0 * channel_gain + 3*j * channel_gain;
    float reference_1010 = 3.0 * channel_gain + 3*j * channel_gain;
    float reference_1011 = 1.0 * channel_gain + 3*j * channel_gain;
    float reference_1100 = -3.0 * channel_gain + 1*j * channel_gain;
    float reference_1101 = -1.0 * channel_gain + 1*j * channel_gain;
